...
async def advance_health_flows(parent_id: str) -> list[dict]:
    ...
    Called each morning before start_daily_conversation() so that:
      ...
      - stale flows are auto-resolved after 3 days without complaint
    ...
    for flow in flows:
        ...
        # Alert child after 7 days unwell, then auto-resolve (safety net)
        if day_count > 7:
            try:
                parent_rows = db.table("parents").select("nickname, family_id").eq("id", parent_id).execute().data
                if parent_rows:
                    nickname = parent_rows[0]["nickname"]
                    family_id = parent_rows[0]["family_id"]
                    condition = flow.get("condition", "health issue")
                    children = db.table("children").select("phone").eq("family_id", family_id).execute().data or []
                ...
                logger.error(f"7-day health alert failed: {alert_err}")
        ...
        try:
            ...
            active_flows.append(flow)  # keep in list so morning greeting still contextualises
...
def open_health_flow(
    ...
    try:
        # Check if an active flow for this condition exists
        existing = (
            db.table("health_flows")
            .select("id")
            .eq("parent_id", parent_id)
